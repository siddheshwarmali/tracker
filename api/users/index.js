...
function canManage(me) {
  return me && (me.role === 'admin' || (me.permissions && me.permissions.userManager));
}
...
module.exports = async (req, res) => {
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (all.users[userId]) return json(res, 409, { error: 'User already exists' });
      ...
      return json(res, 200, { ok: true, user: all.users[userId] });
    }
